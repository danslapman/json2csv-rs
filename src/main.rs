...
use crate::schema::{JsonPath, JsonSchema, extract, generate_tuples, to_schema, json_path_string};
...
fn main() -> io::Result<()> {
    ...
    for line in reader.lines() {
        let line_value = serde_json::from_str::<Value>(line?.as_str()).expect(format!("Can't parse line {}", n).as_str());
